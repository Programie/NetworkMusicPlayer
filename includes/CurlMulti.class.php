...
class CurlMulti
{
	...
	{
		Logger::debug("Starting requests", "CurlMulti/subExec");
		...
		{
			Logger::debug("Waiting " . $retryWait . " seconds to retry failed requests (" . $retry . ")", "CurlMulti/subExec");

			sleep($retryWait);

			Logger::debug("Retrying failed requests (" . $retry . ")", "CurlMulti/subExec");
	...
	{
		Logger::debug("maxRequests = " . $maxRequests . ", retryCount = " . $retryCount . ", retryWait = " . $retryWait, "CurlMulti/exec");
		...
		{
			Logger::debug("Starting exec chunk " . ($index + 1) . " of " . count($instanceChunks), "CurlMulti/exec");
	...
	public function getContent($instanceName)
	{
		...
		return $curlInstance->getContent();
	}
